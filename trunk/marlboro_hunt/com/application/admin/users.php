...
class users extends Admin{
	...
	function home(){
		...
		$filter .= $search=='' ? "" : "AND (name LIKE '%{$search}%' OR last_name LIKE '%{$search}%' OR username LIKE '%{$search}%' OR nickname LIKE '%{$search}%') ";
		...
		$sql = "SELECT count(*) total FROM social_member WHERE n_status= 1 
				AND image_profile IS NOT NULL AND image_profile <> '' {$filter}";
		...
		/* LIST USER MEMBER */
		$sql = "SELECT * FROM social_member WHERE n_status= 1 
				AND image_profile IS NOT NULL AND image_profile <> '' {$filter} ORDER BY register_date DESC

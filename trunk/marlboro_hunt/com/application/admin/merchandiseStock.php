...
class merchandiseStock extends Admin{
	...
	function add(){	
		...
		$data['letter'] 		= $this->_p('letter');		
		$data['postdate'] 	= $this->_p('postdate');
		
		if($this->roler['approver']) $data['n_status'] = $this->_p('n_status');
		else $data['n_status'] 	 = 0;
		
		if($this->roler['approver']) $data['merchandise_type'] = $this->_p('merchandise_type');
		else $data['merchandise_type'] 	 = 0;
		...
		if($this->_p('simpan')){		
			...
			$sql = "INSERT INTO {$this->dbclass}_merchandise (name,detail,stock,letter,postdate,n_status,merchandise_type) 
			VALUES ('{$name}','{$detail}','{$stock}','{$letter}','{$postdate}','{$n_status}','{$merchandise_type}')";
			$this->query($sql);
			// pr($sql);exit;
	...
	function edit(){
		...
		if(! $this->_p('simpan')){
			...
			$stock 			= $this->_p('stock');
			$letter 		= $this->_p('letter');
			$postdate 		= $this->_p('postdate');
			
			if($this->roler['approver']) $status = $this->_p('n_status');
			else $status 	 = 0;
			
			if($this->roler['redeem']) $merch = $this->_p('merchandise_type');
			else $merch 	 = 0;
	...
	function createImage($last_id=null){
				...
				if ($_FILES['image']['name']!=NULL) {
					if ($_FILES['image']['size']>2000000) return false;
	...
	function inputImageThumb($id,$img){
		$this->query("UPDATE {$this->dbclass}_merchandise SET image_thumb='{$img}' WHERE id={$id} ");
	}

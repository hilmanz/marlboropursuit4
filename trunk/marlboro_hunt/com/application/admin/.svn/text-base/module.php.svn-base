...
class module extends Admin{
	...
	function hapus(){
		$requestID = $this->_g('requestID');
		if( !$this->query("DELETE FROM {$this->dbclass}_module_registry WHERE requestID='{$requestID}'")){
			return $this->View->showMessage('Failed Delete Module',"index.php?s={$this->folder}");
		}else{
			return $this->View->showMessage('Success Delete Module',"index.php?s={$this->folder}");
		}
	}

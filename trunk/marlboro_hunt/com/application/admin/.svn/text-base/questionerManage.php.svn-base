...
class questionerManage extends Admin{
	var $category;
	var $type;
	...
	function home(){
		...
		$filter = $search!='' ? " WHERE question LIKE '%{$search}%' " : ""; 
		$start = intval($this->Request->getParam('st'));
		$total_per_page = 20;
		$sql = "SELECT * FROM question {$filter} LIMIT {$start},{$this->total_per_page}";
		$totalSql = "SELECT count(*) total FROM question {$filter}";
		$this->open(0);
	...
	function add(){	
		...
		$data['question'] 	= $this->_p('question');
	
		$content			= $this->_p('content');
		
		if($this->roler['approver']) $data['question_type'] = $this->_p('question_type');
		else $data['question_type'] 	 = 0;
		...
		if($this->_p('simpan')){		
			...
			$sql = "INSERT INTO question (question, question_type, n_status) VALUES ('{$question}', '{$question_type}', '{$n_status}')";
			$this->query($sql);
			// pr($sql);
	...
	function edit(){
		...
		if(! $this->_p('simpan')){
			...
			$id 			= $this->_p('id');
			$question	= $this->_p('question');
			
			
			if($this->roler['approver']) $statusCoba = $this->_p('question_type');
			else $statusCoba 	 = 0;
			...
			$sql = "UPDATE question SET question='{$question}', question_type='{$statusCoba}', n_status='{$status}' WHERE id={$id} LIMIT 1";
		...
		$sql = "SELECT * FROM question_type WHERE n_status = 1";
		$qData = $this->fetch($sql,1);
		
		$this->View->assign("question_type",$qData);
	...
	function hapus(){
		$id = $this->_g('id');
		if( !$this->query("UPDATE question SET n_status=3 WHERE id={$id}")){

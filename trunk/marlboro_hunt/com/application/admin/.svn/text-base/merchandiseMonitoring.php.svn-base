...
class merchandiseMonitoring extends Admin{
	var $category;
	var $type;
	function __construct(){	
		...
		$this->contentType = "0";
		$this->folder =  'merchandiseMonitoring';
	...
	function home(){
		...
		$list_totalComment = $this->fetch("SELECT contentid,COUNT(contentid) AS total_comment FROM marlborohunt_news_content_comment GROUP BY contentid",1);	
		...
		/* list article */
		$sql = " SELECT * FROM my_merchandise ";
		//pr($sql);
	...
	$id = $this->_g('id');
	$sql = "SELECT COUNT(*) AS total_comment FROM marlborohunt_news_content_comment WHERE id={$id};";
	$type = $this->fetch($sql);
	return $type;
	}
	...
	function edit(){
		...
		if(! $this->_p('simpan')){
			...
			$merchandiseid	= $this->_p('merchandiseid');
			$redeemdate 	= $this->_p('redeemdate');
			$name 			= $this->_p('name');
			$address 		= $this->_p('address');
			$phonenumber 	= $this->_p('phonenumber');
			$email 			= $this->_p('email');
			$n_status	 	= $this->_p('n_status');
			
			
			$sql = "UPDATE marlborohunt_news_content SET 	id='{$id}',
														userid='{$userid}',

...
class news extends Admin{
	...
	function comment($cid=null,$all=false,$start=0,$limit=5){
		...
		$sql = "
			SELECT title, userid, date, {$this->dbclass}_news_content_comment.comment
			FROM {$this->dbclass}_news_content
			LEFT JOIN {$this->dbclass}_news_content_comment ON {$this->dbclass}_news_content.id = {$this->dbclass}_news_content_comment.contentid
			WHERE {$this->dbclass}_news_content.id={$id} ";	

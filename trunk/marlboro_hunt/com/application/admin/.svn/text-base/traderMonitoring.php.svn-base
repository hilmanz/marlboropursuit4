...
class traderMonitoring extends Admin{
	var $category;
	var $type;
	function __construct(){	
		parent::__construct();	
		
		$this->type = "3,6,17,18,19,20,21,22,23";
		$this->contentType = "1";
		$this->folder =  'traderMonitoring';
	...
	function home(){
		...
		$filter .= $startdate=='' ? "" : "AND tct.datetime  >= '{$startdate}' ";
		$filter .= $enddate=='' ? "" : "AND tct.datetime  < '{$enddate}' ";		
		$filter .= $search=='' ? "" : "AND (sm.name LIKE '%{$search}%' OR tct.sourceCode LIKE '%{$search}%' OR tct.targetCode LIKE '%{$search}%') ";
		...
		$sql = "SELECT count(*) total
				...
				LEFT JOIN social_member sm ON tct.userid = sm.id
				LEFT JOIN tbl_code_detail tcd ON tct.sourceCode = tcd.id WHERE 1 {$filter}";
		...
		/* list article */
		$sql = "SELECT tct.id, sm.name, tct.activity, tct.sourceCode, tct.targetCode, tct.otheruserid, tct.datetime 
				FROM tbl_code_trade tct
				LEFT JOIN social_member sm ON tct.userid = sm.id
				LEFT JOIN tbl_code_detail tcd ON tct.sourceCode = tcd.id WHERE sm.name IS NOT NULL {$filter}
				GROUP BY tct.datetime ORDER BY tct.datetime DESC LIMIT {$start},{$this->total_per_page} ";
		// pr($sql);
	...
	function add(){	
		...
		if($this->_p('simpan')){		
			...
			if(!$last_id){
				...
				sleep(1);
				// insert task list into log table //
				$this->log->sendActivity("inputtask", 'New task available');

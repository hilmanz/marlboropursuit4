...
class weeklyDecisionManagement extends Admin{
	var $category;
	var $type;
	function __construct(){	
		...
		$this->type = "4";
		$this->contentType = "4";
		$this->folder =  'weeklyDecisionManagement';
	...
	function home(){
		...
		$sql = "
			...
			AND con.articleType IN ({$this->type})
			{$filter}
			GROUP BY con.posted_date, con.created_date
	...
	function add(){	
		...
		//condition
			//first
			$data['condition'][0]['button']= $this->_p('firstconditionbutton');
			$data['condition'][0]['description']= $this->_p('firstcondition');
			//second
			$data['condition'][1]['button']= $this->_p('secondconditionbutton');
			$data['condition'][1]['description']= $this->_p('secondcondition');
				
		$data['topcontent'] 	= $this->_p('topcontent');
		$data['brief'] 			= $this->_p('brief');
		$content				= $this->_p('content');
		// $data['content'] 	  	= $this->fixTinyEditor( $content );
		$data['content'] 	  	= $this->_p('title');
	...
	function edit(){
		...
		if(! $this->_p('simpan')){
			...
			// pr($qData);
			if($qData){
				if($qData['tags']!='')	$qData['tags'] =unserialize($qData['tags']);
			...
			//condition
			//first
				$condition[0]['button']= $this->_p('firstconditionbutton');
				$condition[0]['description']= $this->_p('firstcondition');
				//second
				$condition[1]['button']= $this->_p('secondconditionbutton');
				$condition[1]['description']= $this->_p('secondcondition');

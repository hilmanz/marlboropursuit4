...
class uCategoryHelper {
	...
	function allUserRegistration(){
		...
		$sql = "SELECT COUNT(*) num, DATE(log.date_time) dd 
				FROM tbl_activity_log log
				LEFT JOIN social_member sm ON sm.id=log.user_id
				WHERE 
				sm.n_status = 1 
				AND log.action_id = 1 
				AND DATE(log.date_time) >= '{$this->startdate}' 
				AND DATE(log.date_time)  <= '{$this->enddate}' 
				GROUP BY dd 
				ORDER BY dd ASC"; 
				// pr($sql);
		// $this->apps->open(1);
	...
	function userUnverified(){
		$sql = "SELECT count( * ) num, DATE( register_date ) dd
				FROM social_member WHERE n_status <> 1  AND DATE(register_date) >= '{$this->startdate}'
				AND DATE(register_date) <= '{$this->enddate}' GROUP BY dd	ORDER BY dd ASC "; 
		// $this->apps->open(1);
		$qData = $this->apps->fetch($sql,1);
		if($qData){
		$newdata = $this->webActivityHelper->fixeddate($qData,'dd','num');
	...
	function activeUser(){
		...
		$sql = "SELECT count(*) num, DATE(date_time) datetime
				FROM tbl_activity_log log 
				LEFT JOIN social_member sm ON log. user_id = sm.id
				WHERE log.action_id = 1 AND sm.login_count >= 1 AND DATE(date_time) >= '{$this->startdate}' AND  sm.n_status = 1
				AND DATE(date_time) <= '{$this->enddate}' GROUP BY datetime ORDER BY datetime ASC"; 
		// $this->apps->open(1);
		$qData = $this->apps->fetch($sql,1);
		if($qData){
